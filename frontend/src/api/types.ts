...
export type CrossDomainCategory = 'billing_integrity' | 'safety_pattern' | 'vendor_operational_divergence' | 'shift_readiness_pattern' | 'data_integrity_anomaly';
export interface CrossDomainSignal { metric: string; current_value: number | null; baseline_value: number | null; peer_median: number | null; relative_change_pct: number | null; direction: 'worse' | 'better' | 'stable'; weight: number; note: string | null }
export interface RiskComponent { name: string; value: number; detail: string }
export interface CrossDomainAnomaly { id: string; category: CrossDomainCategory; entity_type: string; entity_name: string; title: string; severity: 'low' | 'medium' | 'high'; confidence: string; cross_signal_risk_score: number; signals: CrossDomainSignal[]; why_flagged: string; recommended_investigation: string[]; risk_components: RiskComponent[]; sample_size: number; month: string; baseline_month: string | null; supporting_dimensions: Record<string, unknown>; data_quality_warnings: string[] }
export type Investigable = Anomaly | CrossDomainAnomaly;
export const isCrossDomain = (a: Investigable): a is CrossDomainAnomaly => 'signals' in a && 'cross_signal_risk_score' in a;
